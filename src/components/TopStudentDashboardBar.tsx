...
export const TopStudentDashboardBar: React.FC<TopStudentDashboardBarProps> = ({
  ...
  return (
    <div className="bg-white/95 dark:bg-slate-900/95 border border-slate-200/90 dark:border-slate-800 rounded-3xl shadow-sm backdrop-blur-md transition-all text-slate-800 dark:text-slate-100 overflow-hidden">
      ...
      <div className="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-3 sm:gap-4">
        
        {/* Student Profile Identity Tag */}
        <div className="flex items-center space-x-3 w-full xl:w-auto justify-between xl:justify-start min-w-0">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate">
                ...
                {studentProfile.entryMode && (
                  <span className="ml-1 text-[10px] px-1.5 py-0.2 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                    {studentProfile.entryMode === 'Direct_Entry' ? 'DE' : 'UTME'}
                  </span>
                )}
              </p>
            </div>
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-2 shrink-0">
            {onOpenProgrammeRulesModal && (
              <button
                ...
                className="p-2 rounded-xl bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/60 dark:hover:bg-blue-900/60 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 transition-colors flex items-center space-x-1 text-xs font-semibold cursor-pointer"
            ...
            <button
              ...
              className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors flex items-center space-x-1 text-xs font-semibold cursor-pointer"
        ...
        <div className="flex items-center space-x-2 sm:space-x-2.5 w-full xl:w-auto justify-start xl:justify-end flex-wrap gap-y-2">
          
          {/* FUTMinna / NUC Compliance Status Badge & Trigger */}
          {onOpenCompliance && (
            <button
              onClick={onOpenCompliance}
              className="flex items-center space-x-2 px-2.5 sm:px-3 py-1.5 rounded-xl border text-left shrink-0 transition-all bg-emerald-50/90 dark:bg-emerald-950/60 border-emerald-200 dark:border-emerald-800 text-emerald-900 dark:text-emerald-200 hover:bg-emerald-100 cursor-pointer"
          ...
          {onOpenSiwesPortal && (
            ...
            >
              <div className={`p-1 rounded-md shrink-0 ${siwesAudit.isEligibleNow ? 'bg-blue-600 text-white' : 'bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-400'}`}>
          ...
          <div className="flex items-center space-x-2 bg-indigo-50/80 dark:bg-indigo-950/50 border border-indigo-200/80 dark:border-indigo-800/80 px-2.5 sm:px-3 py-1.5 rounded-xl shrink-0">
            ...
            <div className="text-left">
              ...
              <span className="text-xs font-extrabold text-slate-900 dark:text-white leading-tight truncate block max-w-[150px] sm:max-w-none">
                {targetTrack.title}
              </span>
            </div>
          </div>
          ...
          <button
            ...
            className="flex items-center space-x-1.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-3 sm:px-3.5 py-2 rounded-xl text-xs font-bold transition-all shadow-xs shrink-0 cursor-pointer"
